...
This document presents the requirements for the \emph {TimeMate}. \emph {TimeMate} is a system whose purpose and main functionality is to administer time reporting with web-usage capabilities. 
...
\begin{enumerate}
  ...
  \item The requirements of \emph{BaseBlockSystem} applies to this document and are therefore implemented in \emph {TimeMate}. The following requirements will \emph{not} be implemented; 6.2.3, 6.3.4
...
If the user has forgotten their password, they should receive a newly generated password from the server to the email connected to their username.

Users who have forgotten their passwords should be able to receive from the server a newly generated password to the email connected to their username. The users current password is then overwritten with the new password.

\subsubsection{Requirement}
New users should be assigned newly generated passwords by the server and receive them by email.
...
\subsubsection{Requirement}
Project leaders should be able to create and submit a time report.
